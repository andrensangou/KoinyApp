...

      {
        mainView === 'profile' && (
          <div className="fixed inset-0 bg-slate-100 dark:bg-slate-950 z-[40] flex flex-col p-4 animate-fade-in pt-24 overflow-y-auto no-scrollbar">
            <div className="w-full max-w-2xl mx-auto rounded-[2.5rem] bg-white shadow-none sm:shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800">
                  ...
                  settingsView === 'LIST' ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {data.children && data.children.map(child => (
                          <div key={child.id} className="flex items-center justify-between p-4 bg-white rounded-3xl shadow-sm border border-slate-100/50 group hover:shadow-md transition-all">
                            <div className="flex items-center gap-4">
                              <div className="w-14 h-14 bg-slate-50 rounded-2xl p-1 shadow-inner ring-1 ring-slate-100 flex items-center justify-center">
                                {renderAvatar(child.avatar, "w-full h-full", child.colorClass)}
                              </div>
                              <div>
                                <p className="font-black text-slate-800 text-sm tracking-tight">{child.name}</p>
                      ...
                      <button onClick={startAddChild} className="w-full py-6 border-2 border-dashed border-slate-200 bg-white/50 rounded-3xl text-slate-400 font-black uppercase tracking-[0.2em] text-[10px] hover:border-indigo-300 hover:text-indigo-500 hover:bg-white transition-all flex items-center justify-center gap-3 group active:scale-[0.98]">
                        <i className="fa-solid fa-user-plus text-lg opacity-40 group-hover:opacity-100 transition-opacity"></i>
                        {t.parent.addChild}
                      </button>
                    </div>
                  ) : (
                    <form onSubmit={saveChildForm} className="space-y-6 text-slate-900 animate-fade-in-up">
                      <div className="flex flex-col items-center mb-2">
                        <div className="bg-white p-4 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-50 flex flex-col items-center w-40 relative overflow-hidden">
                          ...
                          <span className="font-black text-slate-800 text-sm uppercase tracking-widest truncate w-full text-center px-2">{formName || t.parent.childName}</span>
                        </div>
                      </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                              <select
                                ...
                                className="flex-1 p-4 border-2 border-slate-50 rounded-2xl bg-slate-50/50 focus:border-indigo-500 focus:bg-white focus:ring-4 focus:ring-indigo-50 outline-none text-slate-900 font-black transition-all appearance-none"
                              >
                      ...
                      <div className="space-y-3 relative">
                        ...
                        <button
                          ...
                          className="w-full p-3 bg-white border border-slate-200 rounded-2xl flex items-center justify-between hover:border-indigo-300 transition-all shadow-sm group"
                        >
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10">
                              {renderAvatar(formAvatar, "w-full h-full", formColorClass)}
                            </div>
                            <span className="font-bold text-slate-700 tracking-wide">{formAvatar}</span>
                          </div>
                        ...
                        {isAvatarDropdownOpen && (
                          <>
                            ...
                            <div className="absolute top-full left-0 right-0 mt-2 p-4 bg-white border border-slate-100 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.15)] z-[60] animate-scale-in">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative group">
                          <select
                            ...
                            className="w-full p-4 bg-white border border-slate-200 rounded-2xl appearance-none focus:ring-2 focus:ring-indigo-500 outline-none font-bold text-slate-700 shadow-sm pr-12"
                          >
                      ...
                      <div ref={formGoalsRef} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                        <div className="p-5 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
                          ...
                          <button type="button" onClick={handleAddGoalToForm} className="bg-white text-indigo-600 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:shadow-md transition-all active:scale-95 flex items-center gap-2 border border-indigo-100 shadow-sm">
                          ...
                          {formGoals.length === 0 ? (
                            ...
                            <div key={goal.id} className="flex flex-wrap sm:flex-nowrap items-center gap-3 p-3 bg-slate-50/50 rounded-2xl border border-slate-100/50 animate-scale-in group">
                              ...
                              <input
                                ...
                                className="flex-1 min-w-[120px] p-3 rounded-xl border-2 border-transparent bg-transparent text-sm font-black outline-none focus:border-indigo-100 focus:bg-white transition-all"
                              />
                              <div className="relative">
                                <input
                                  type="number"
                                  ...
                                  placeholder="0"
                                  className="w-24 p-3 pr-8 rounded-xl border-2 border-transparent bg-transparent text-sm font-black text-right outline-none focus:border-indigo-100 focus:bg-white transition-all"
                                />
                ...
                {activeTab === 'ACCOUNT' && (
                  <div className="space-y-4 max-w-lg mx-auto pb-8 text-slate-900 dark:text-white transition-colors duration-500">
                    ...
                    <div className="space-y-3 text-slate-900 dark:text-white">
                      ...
                      <button onClick={() => setShowHelp(true)} className="w-full flex items-center justify-between p-4 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-2xl shadow-sm border border-indigo-100 dark:border-indigo-900/30 group transition-all hover:shadow-md text-left">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 text-indigo-500 dark:text-indigo-400 flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
                            <i className="fa-solid fa-book-open"></i>
                          </div>
                          <span className="font-bold text-indigo-700 dark:text-indigo-300 text-sm">{t.common.userGuide}</span>
                        </div>
                        <i className="fa-solid fa-chevron-right text-indigo-300 dark:text-indigo-600 group-hover:translate-x-1 transition-transform"></i>
                      </button>
