...
4. **Avis in-app** (`SKStoreReviewRequest` iOS + Google In-App Review Android, via `capacitor-rate-app`) — **à bundler dans le même build**. Implémenter le prompt natif (déclencheur retenu : **3ème `app_open`** via compteur `koiny_app_open_count` localStorage, idéalement après un moment positif type validation de mission). **Conforme Apple + RGPD** (popup natif, pas d'email → contourne le souci `marketing_consent=false`). Capte les satisfaits (laetitia = power user idéale). Apple limite à 3 prompts/an auto. Cf. section "📋 À FAIRE (session future) — Demande d'avis in-app" plus bas pour le détail. **Pourquoi maintenant pertinent** : c'est LE seul moyen conforme d'obtenir des avis (l'email est bloqué par le manque d'opt-in marketing).
