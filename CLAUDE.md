...
- **✅ PREUVE CONCRÈTE (compte dev nsjdre `af12ec91`)** : iPhone avec toggle "Activer les Notifications Push" = **ON**, mais en base **753 tokens, 100% `android`, 0 iOS** → confirme que le token iOS n'est JAMAIS écrit (toggle local ≠ token enregistré). Et les 753 tokens sont **tous datés du 07/06 entre 06:04 et 06:15 (11 min)** → `registerPushToken()` appelé en boucle pendant la période de churn/reload-loop du 07-08/06 (foreground reload + polling 5s) → insert au lieu d'upsert → accumulation runaway. **Les 2 bugs sont donc liés au même symptôme : pas d'upsert + appels répétés, et un chemin iOS qui ne persiste rien.**
- **📊 ANDROID vs iOS — priorités révisées (distribution 28/06)** : Android **enregistre bien** ses tokens (≠ iOS). L'accumulation est surtout **historique** : pic de **656 tokens le 07/06** (bug churn, corrigé le 08/06), puis effondrement → **seulement 11 tokens depuis le 20/06**. Vrais users = bloat modéré (mireille 36, luisa 15, ff1788a/lualsanher 14, autres 1-8). → **iOS = 🔴 prioritaire** (0 token, payants injoignables). **Android = 🟡 nettoyage non urgent** (purge du bloat historique + upsert pour l'hygiène, mais ça ne casse rien aujourd'hui). Le no-upsert reste à corriger des 2 côtés.

**À faire sur `fix/push-ios-tokens`** :
1. **✅ FAIT + VALIDÉ (28/06) — iOS token** : migré de `@capacitor/push-notifications` vers **`@capacitor-firebase/messaging`** (`FirebaseMessaging.getToken()` → vrai token FCM sur iOS ET Android). `FirebaseApp.configure()` ajouté dans AppDelegate. `@capacitor/push-notifications` désinstallé. Stub `firebase/messaging` (alias Vite, même pattern qu'analytics) pour le build web. **Testé sur iPhone physique** → 1 token `ios/parent` enregistré (1ère fois). Commits `969a01c`, `7701f0c`.
2. **✅ FAIT + APPLIQUÉ EN PROD (28/06) — accumulation** : cause = contrainte UNIQUE inopérante (NULL≠NULL pour child_id en mode parent). Migration `20260628_device_tokens_dedup_unique.sql` (dédup ROW_NUMBER + `CREATE UNIQUE INDEX ... NULLS NOT DISTINCT`) **exécutée dans SQL Editor**. Résultat : **874 → 22 tokens, 0 doublon**. nsjdre 753→6. L'upsert du code marche enfin. Listener `tokenReceived` aussi attaché 1× (anti-fuite côté code).
...
4. **✅ FAIT (28/06) — Avis in-app** : ⚠️ `capacitor-rate-app` (suggéré initialement) est **INCOMPATIBLE Capacitor 8** (peer `^5.0.0`) → utilisé **`@capacitor-community/in-app-review@8.0.0`** (peer `>=8.0.0`, `InAppReview.requestReview()` → `SKStoreReviewRequest` iOS + In-App Review Android). **`services/appReview.ts` (créé)** : `incrementAppOpen()` (compteur `koiny_app_open_count` localStorage, appelé au mount dans `App.tsx`) + `maybeRequestReview()` (déclenché dans `handleApprove` après validation de mission, +1.5s pour laisser l'animation de succès). **Gardes-fous** : ≥3 ouvertures + 1 seule fois (`koiny_review_asked`) + natif only ; Apple/Google brident à ~3/an. **Déclencheur = moment positif (validation mission) + user engagé** → cible les satisfaits (laetitia). **Conforme Apple + RGPD** (popup natif, pas d'email → contourne `marketing_consent=false`, seul moyen conforme d'avoir des avis). Typecheck OK, à builder dans build 21. ⚠️ Le prompt natif Apple ne s'affiche pas toujours en dev/à la demande (throttle Apple) → validation réelle en prod.
...
**Statut:** **🚀 iOS 1.1.7 build 21 SOUMIS le 28/06/2026** (branche `fix/push-ios-tokens`) — bundle onboarding + **fix push iOS (token FCM)** + avis in-app + REST-bypass. Build 20 avait été retiré du review → remplacé par build 21. Android **versionCode 18 LIVE en prod** (publié 28/06, contient REST-bypass + onboarding ; le fix push iOS ne concerne pas Android dont le push marchait déjà). Précédent : iOS 1.1.6 build 19, Android vc17.

⚠️ **Compte review Apple (`akians237@gmail.com`) — piège à connaître** : il a été supprimé puis recréé via Google pendant les tests du 28/06 → la **connexion directe email/mot de passe ne marchait plus** (provider `google` only) ET le **PIN avait changé**. Fix appliqué via admin API : (1) `PUT /auth/v1/admin/users/{id}` avec `password=KoinyReview2024` → réactive la connexion directe (testé via `token?grant_type=password` → OK) ; (2) `pin_hash` recalculé pour PIN `0000` (PBKDF2 SHA-512, 100k, salt 16B, format `salt:hash` — cf. `services/security.ts`) via service role. **Leçon : ne jamais supprimer le compte review en testant, ou penser à re-set password + PIN avant de soumettre.**
