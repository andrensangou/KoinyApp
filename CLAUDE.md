...
- **⏳ RESTE (à faire ~29/06) — marquer `child_created` comme événement clé** : GA4 → Admin → Affichage des données → **Événements** → onglet **Événements clés**. L'event `child_created` n'apparaissait pas encore le 28/06 (délai GA4 24-48h + build live tout juste sorti). À faire : soit "Nouvel événement clé" → taper `child_created`, soit attendre qu'il remonte dans "Événements récents" puis cliquer l'⭐. Une fois événement clé → conversion auto-dispo dans Google Ads. (Optionnel : `sign_up`, `purchase` aussi.)
